...
$police_id = (int)$_SESSION['police_id'];

// Handle delete (police can only delete their own)
if (isset($_GET['delete'])) {
    $id = (int)$_GET['delete'];
    $res = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM news WHERE id=$id AND posted_by=$police_id AND posted_by_role='police'"));
    if ($res) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['post_news'])) {
    ...
    $name     = mysqli_real_escape_string($conn, $_SESSION['police_name'] ?? 'Police');
    $sid      = isset($_SESSION['station_id']) ? (int)$_SESSION['station_id'] : 'NULL';
    $image_name = NULL;
...
$result = mysqli_query($conn, "SELECT * FROM news WHERE posted_by=$police_id AND posted_by_role='police' ORDER BY created_at DESC");
?>
<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<title>Daily News | Police Panel</title>
...
.form-group input, .form-group select, .form-group textarea {
    width:100%; padding:9px 12px; border:1px solid #ddd; border-radius:5px; font-size:13px; box-sizing:border-box;
}
...
.news-title { font-weight:700; font-size:14px; color:#222; }
.news-meta { font-size:11px; color:#888; margin:3px 0 6px; }
...
.btn-sm { padding:4px 10px; border-radius:4px; font-size:12px; text-decoration:none; border:none; cursor:pointer; }
...
<div class="main-content">
  <div class="page-header"><h2>Daily News</h2></div>

  <?php if ($success): ?><div class="alert-success"><?= $success ?></div><?php endif; ?>
  <?php if ($error):   ?><div class="alert-error"><?= $error ?></div><?php endif; ?>
  <?php if (isset($_GET['msg']) && $_GET['msg']==='deleted'): ?>
    <div class="alert-success">News deleted.</div>
  <?php endif; ?>
      ...
      <?php else: ?>
        <?php while ($row = mysqli_fetch_assoc($result)): ?>
          ...
          <div class="news-item">
            ...
            <div class="news-meta">
              <span class="badge badge-<?= $cat_class ?>"><?= $row['category'] ?></span>
              &nbsp;<?= date('d M Y', strtotime($row['created_at'])) ?>
              &nbsp;<?= $row['is_active'] ? '<span style="color:green;">●Active</span>' : '<span style="color:#ccc;">●Hidden</span>' ?>
            </div>
